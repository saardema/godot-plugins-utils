...
func _add_to_min_deque(value: float):
	while _min_deque.size() > 0 and _min_deque[0] <= _pointer - _deque_window_size:
		_min_deque.pop_front()

	while _min_deque.size() > 0 and values[_min_deque[-1]] >= value:
		_min_deque.pop_back()

	_min_deque.push_back(_pointer)

func _add_to_max_deque(value: float):
	while _max_deque.size() > 0 and _max_deque[0] <= _pointer - _deque_window_size:
		_max_deque.pop_front()

	while _max_deque.size() > 0 and values[_max_deque[-1]] <= value:
		_max_deque.pop_back()

	_max_deque.push_back(_pointer)
